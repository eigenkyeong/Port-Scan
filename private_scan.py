...
def save_log():
    ...
    fileHandler = logging.FileHandler('./private_scanning.log')
...
def make_ipList():
    ...
    for ip3 in range(256):
        for ip4 in range(256):
            ip_list.append(base_ip + str(ip3) + '.' + str(ip4))
...
def save_result():
    global result_list
    # wb2 = Workbook()
    wb2 = load_workbook("./private_result.xlsx")
    ws2 = wb2.create_sheet("result")
    col_names = ['ip', 'port']
